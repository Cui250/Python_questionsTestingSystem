...
#                 db.session.commit()
#                 return render_template('deletequestion.html', message="题目删除成功")
#             else:
#                 return render_template('deletequestion.html', message="题号不存在--删除失败")
#         else:
#             return render_template('deletequestion.html', message="删除失败")
#
...
#             # 创建一个新的列表来存储修改后的字符串
#             modified_choice_list = []
#             for x in choice_list:
#                 # 如果字符串不为空，则将其第一个字符转换为大写，否则保持原样
#                 if x:
#                     modified_x = x[0].upper() + x[1:]
#                 else:
#                     modified_x = x
#                 modified_choice_list.append(modified_x)
#
#                 # 将新的列表转换回由换行符分隔的字符串
#             choice = "\n".join(modified_choice_list)
#             answer=form.answer.data.upper()
#             difficulty=form.difficulty.data
#             kind=form.kind.data
#             if  answer not in ('A', 'B', 'C','D'):
#                 return render_template('changequestion.html', message="你的修改不符合规范--修改失败")
#             else:
...
#                     db.session.add(Question)
#                     db.session.commit()
#                     return render_template('changequestion.html', message="题目增加成功")
#                 else:
#                     Question = QuestionModel.query.filter_by(id=id).first()
#                     Question.question = question
...
#                     Question.kind = kind
#                     db.session.commit()
#                 return render_template('changequestion.html', message="数据修改成功")
#         else:
#             return render_template('changequestion.html', message="你的修改不符合规范--修改失败")
